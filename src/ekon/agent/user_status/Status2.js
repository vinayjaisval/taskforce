	...
	const Status2 = (props) => {
		...
		useEffect(() => {
			...
			async function getTaskHistory() {
				...
				try {
					const taskHistoryApi = await axios.get(`https://task.mycrmdesk.com/backend/api/admin/task_dashboard_history/${id}/status2`)
			...
			<div className='text-danger'>
			{
				...
				taskHistory.map((item,index)=>(
					<div key={index+1} className='text-primary'>
						<Link to={'/agent/task-log/'+item.lead_id}><b>{item.name}</b></Link> <br />
