  ...
  async function onFormSubmit(e) {
    ...
    document.getElementById("signup-status").style.borderColor = "#f8f9fa";
		...
		if(categoryName.name == ''){
			...
			try {
			axios.post(`https://task.mycrmdesk.com/backend/api/admin/add-category`, categoryName)
			.then((res) => {
				console.log(res);
	...
	useEffect(() => {
		...
		async function getCategoryList(page) {
			page = page;
			try {
				const categoryListApi = await axios.get(`https://task.mycrmdesk.com/backend/api/admin/category?page=`+page)
	...
	async function handleClick(e, delId){
		...
		axios.get(`https://task.mycrmdesk.com/backend/api/admin/category_delete/${delId}`)
		.then((res) => {
		  getPaginatedData(1);
          ...
          <Card stretch>
                ...
                <form className="row" id="interviewForm" >
                  ...
                  <div className='col-12'>
                        ...
                        <option value={categoryName.name}>Select</option>
														...
														<DropdownMenu isAlignmentEnd>
															<DropdownItem>
																<Button icon='Visibility'>
																	<span onClick={e => handleClick(e, item.id)} > <i className="fa fa-trash"></i> Delete Category</span>
																</Button>
															</DropdownItem>
														</DropdownMenu>
													</Dropdown>
