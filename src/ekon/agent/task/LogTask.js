...
const LogTask = () => {
	...
	async function onFormSubmit(e) {
		...
		if(lead.newremarks == '' || lead.newremarks == null){
			...
			try {
			axios.post(`https://task.mycrmdesk.com/backend/api/admin/update-lead-followup/${id}/${userIds}`, lead)
			.then((res) => {
		...
		// Get instant Log
		try {
			const logApi = await axios.get(`https://task.mycrmdesk.com/backend/api/admin/leads-log/${id}`)
	...
	useEffect(() => {
    	...
    	async function getLeadList() {
        	try {
				 ...
				 const array = allLeadsApi.data.assignee.split(',');
				 if( array.indexOf(userIds) < 0){
					window.location.assign('/agent/task.html');
				 }
	...
	async function onTimerCal(e, leadIds){
		...
		try {
			await axios.get(`https://task.mycrmdesk.com/backend/api/admin/timer-calculation/${userIds}/${leadIds}`)
		
			document.getElementById("succ_message1").style.display = "block";
			document.getElementById("alert_message1").innerHTML = "Your Time Updated. Please Refrese Page to see the Changes!!";
			window.scrollTo({ top: 0, behavior: 'smooth' });
		} catch (error) { }
	}
					...
					<div className='col-md-4 col-xs-12 '>
						<Card stretch>
							...
							<CardBody className='row g-4'>
								...
								<h6 className={lead.total_working_time > lead.total_time_assign ? 'text-danger' : 'text-primary'}>
									Task Time:- <b>{ lead.total_time_assign } Min</b><br />
							...
							<CardBody style={{maxHeight: '500px', overflow: 'auto'}}>
								{
									...
									logList.map((item,index)=>(
										...
										<div className='abc' key={index+1}>
											<h6><b>{item.name}</b> change to <b>{item.status_name}</b> at <b>{item.created_at}</b> </h6>
											<p>{item.remarks}</p>
										</div>
										</>
									)) :
