...
const EditSLeadCanva = (props) => {
	...
	async function getLead() {
		try {
			const leadApi = await axios.get(`https://task.mycrmdesk.com/backend/api/admin/edit-lead/${userId}`)
	...
	async function getCountryList() {
		try {
			const countryListApi = await axios.get(`https://task.mycrmdesk.com/backend/api/admin/all-country-list`)
	...
	async function getStateList() {
		...
		try {
			const stateListApi = await axios.get(`https://task.mycrmdesk.com/backend/api/admin/all-state-list?country=${lead.country}`)
	...
	async function getCityList() {
		try {
			const cityListApi = await axios.get(`https://task.mycrmdesk.com/backend/api/admin/all-city-list?state=${lead.state}`)
	...
	async function onTextFieldChange(e) {
		if(e.target.name == 'country'){
			try {
				const stateListApi = await axios.get(`https://task.mycrmdesk.com/backend/api/admin/all-state-list?country=`+e.target.value)
				setStateList(stateListApi.data);
			} catch (error) {
				console.log("Something is Wrong");
			}
		...
		if(e.target.name == 'state'){
			try {
				const cityListApi = await axios.get(`https://task.mycrmdesk.com/backend/api/admin/all-city-list?state=`+e.target.value)
				setCityList(cityListApi.data);
			} catch (error) {
				console.log("Something is Wrong");
			}
	...
	async function onFormSubmit(e) {
		e.preventDefault()
		try {
			axios.put(`https://task.mycrmdesk.com/backend/api/admin/update-lead/${userId}`, lead)
			.then((res) => {
				...
				<form className='row g-4' id='leadForm'>
					...
					<div className='col-12'>
						<FormGroup
							...
							isFloating
							label='Source'>
							<select name="source" id="source" className="form-control select2" onChange={e => onTextFieldChange(e)} >
					...
					<div className='col-12'>
						<FormGroup
							...
							isFloating
							label='Country'>
							<select className="form-control" name="country" id="country" onChange={e => onTextFieldChange(e)}>
					...
					<div className='col-12'>
						<FormGroup
							...
							isFloating
							label='State'>
							<select name="state" id="state" className="form-control select2" onChange={e => onTextFieldChange(e)} >
					...
					<div className='col-12'>
						<FormGroup
							...
							isFloating
							label='City'>
							<select name="city" id="city" className="form-control select2" onChange={e => onTextFieldChange(e)} >
