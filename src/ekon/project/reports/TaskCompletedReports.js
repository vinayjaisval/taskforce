...
const TaskCompletedReports = () => {
		...
		async function getAstroList(page) {
			...
			try {
				const astroListApi = await axios.get(
					`${BASE_URL}/admin/leads_completed_project/${id}?page=` + page,
				);
	...
	async function getPaginatedData(page) {
		...
		try {
			const astroListApi = await axios.get(
				`${BASE_URL}/admin/leads_completed_project/${id}?page=` +
					page +
					`&keywords=` +
					keywordVal,
			);
	...
	async function onTextFieldChange(e) {
		...
		try {
			const astroListApi = await axios.get(
				`${BASE_URL}/admin/leads_completed_project/${id}?page=1&keywords=` + e.target.value,
			);
			setAstroList(astroListApi.data.data);
			setTotalRecords(astroListApi.data.total);
			setLimit(astroListApi.data.limit);
