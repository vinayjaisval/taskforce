...
const PendingTask = () => {
	...
	useEffect(() => {
		...
		async function getAstroList(page) {
			page = page;
			try {
				const astroListApi = await axios.get(`https://task.mycrmdesk.com/backend/api/admin/leads_pending_project/${id}?page=`+page)
	...
	async function getPaginatedData(page){
		...
		try {
			const astroListApi = await axios.get(`https://task.mycrmdesk.com/backend/api/admin/leads_pending_project/${id}?page=`+page+`&keywords=`+keywordVal)
			setAstroList(astroListApi.data.data);
			setTotalRecords(astroListApi.data.total);
			setLimit(astroListApi.data.limit);
			setOffset(astroListApi.data.offset);
	...
	async function onTextFieldChange(e) {
		...
		try {
			const astroListApi = await axios.get(`https://task.mycrmdesk.com/backend/api/admin/leads_pending_project/${id}?page=1&keywords=`+e.target.value)
			setAstroList(astroListApi.data.data);
			setTotalRecords(astroListApi.data.total);
			setLimit(astroListApi.data.limit);
			setOffset(astroListApi.data.offset);
