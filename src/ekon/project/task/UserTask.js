...
const UserTask = () => {
	...
	const { id } = useParams();
	const sessIds = localStorage.getItem('sess_id');
	...
	useEffect(() => {
		...
		async function getAstroList(page) {
			page = page;
			try {
				const astroListApi = await axios.get(`https://task.mycrmdesk.com/backend/api/admin/leads_users_list_project/${id}/${sessIds}?page=`+page)
	...
	async function getPaginatedData(page){
		...
		try {
			const astroListApi = await axios.get(`https://task.mycrmdesk.com/backend/api/admin/leads_users_list_project/${id}/${sessIds}?page=`+page+`&keywords=`+keywordVal)
			setAstroList(astroListApi.data.data);
			setTotalRecords(astroListApi.data.total);
			setLimit(astroListApi.data.limit);
		} catch (error) {
	...
	async function onTextFieldChange(e) {
		...
		try {
			const astroListApi = await axios.get(`https://task.mycrmdesk.com/backend/api/admin/leads_users_list_project/${id}/${sessIds}?page=1&keywords=`+e.target.value)
			setAstroList(astroListApi.data.data);
			setTotalRecords(astroListApi.data.total);
			setLimit(astroListApi.data.limit);
		} catch (error) {
		...
		<PageWrapper title={dashboardMenu.manageAstrologer.subMenu.ManageAstro.text}>
			<SubHeader>
				<SubHeaderLeft>
					<Breadcrumb
						list={[
							{ title: 'Home', to: '/project/dashboard.html' },
							{
														...
														<DropdownMenu isAlignmentEnd>
															...
															<DropdownItem>
																<Button icon='Visibility'>
																	<span onClick={e => handleClick(e, item.id)} > <i className="fa fa-trash"></i> Delete Task</span>
																</Button>
															</DropdownItem>
														</DropdownMenu>
													</Dropdown>
