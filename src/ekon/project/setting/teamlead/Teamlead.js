...
const Teamlead = () => {
  ...
  async function onFormSubmit(e) {
		...
		if(agentName.name == ''){
			...
			try {
			axios.post(`https://task.mycrmdesk.com/backend/api/admin/add-teamlead`, agentName)
			.then((res) => {
				console.log(res);
	...
	useEffect(() => {
		...
		async function getAgentList(page) {
			page = page;
			try {
				const agentListApi = await axios.get(`https://task.mycrmdesk.com/backend/api/admin/teamleads?page=`+page)
	...
	async function handleClick(e, delId){
		...
		axios.get(`https://task.mycrmdesk.com/backend/api/admin/teamlead_delete/${delId}`)
		.then((res) => {
		  getPaginatedData(1);
	...
	return (
		<PageWrapper title={dashboardMenu.masters.subMenu.teamlead.text}>
			<SubHeader>
        <SubHeaderLeft>
          <Breadcrumb
            list={[
