...
const Status = () => {
	...
	async function onFormSubmit(e) {
		...
		if (sourceName.name == '') {
			...
			try {
				axios.post(`${BASE_URL}/admin/add-source`, sourceName).then((res) => {
					console.log(res);
	...
	useEffect(() => {
		async function getSourceList(page) {
			page = page;
			try {
				const sourceListApi = await axios.get(`${BASE_URL}/admin/source?page=` + page);
	...
	async function handleClick(e, delId) {
		axios.get(`${BASE_URL}/admin/source_delete/${delId}`).then((res) => {
			getPaginatedData(1);
