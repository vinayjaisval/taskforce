...
const EditProject = () => {
	...
	async function onFormSubmit(e) {
		...
		if (lead.name == '') {
			...
			try {
				axios.put(`${BASE_URL}/admin/update-project/${id}`, lead).then((res) => {
	...
	useEffect(() => {
		...
		*/
		...
		async function getSkillsList() {
			try {
				const skillListApi = await axios.get(`${BASE_URL}/admin/all-skills-list`);
	...
	var onSelect = async (e) => {
		...
		// Sel Assignee list
		try {
			const selAssApi = await axios.get(
				`${BASE_URL}/admin/get_sel_assignee_by_skills/${skills}`,
			);
	...
	useEffect(() => {
		async function getDepartments() {
			try {
				const departmentApi = await axios.get(`${BASE_URL}/admin/edit-project/${id}`);
				setLead(departmentApi.data);

				// Assignee list
				try {
					const selAssApi = await axios.get(
						`${BASE_URL}/admin/get_sel_assignee_by_skills/${departmentApi.data.skills}`,
					);
					setAssigneeList(selAssApi.data.data);
				} catch (error) {
					console.log('Something is Wrong -Sel Skills List');
				...
				try {
					const selAssApi = await axios.get(
						`${BASE_URL}/admin/get_sel_assignee/${departmentApi.data.assignee}`,
					);
					setSelAssigneeList(selAssApi.data.data);
				} catch (error) {
				...
				// Sel Skills list
				try {
					const selSkillApi = await axios.get(
						`${BASE_URL}/admin/get_sel_skills/${departmentApi.data.skills}`,
					);
					setSelSkillsList(selSkillApi.data.data);
				} catch (error) {
					console.log('Something is Wrong -Sel Subject List');
				}
			} catch (error) {
							...
							<CardBody className=''>
								...
								<form className='row g-4' id='leadForm'>
									<div className='col-md-4 col-xs-12'>
										<FormGroup id='signup-name' isFloating label='Project Name'>
											<Input
												type='text'
												autoComplete='name'
												name='name'
									...
									<div className='col-md-4 col-xs-12'>
										<FormGroup id='signup-email' isFloating label='Email Id'>
											<Input
												type='text'
												autoComplete='email'
												name='email'
									...
									<div className='col-md-6 col-xs-12 label-hide'>
										<FormGroup id='lead-skills' isFloating label='--Skills--'>
											<Multiselect
												options={skillsList}
