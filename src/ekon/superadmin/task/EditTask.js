...
const EditTask = () => {
	...
	async function onTextFieldChange(e) {
		...
		if(e.target.name == 'project'){
			try {
				const selAssApi = await axios.get(`https://task.mycrmdesk.com/backend/api/admin/get_sel_assignee_by_project/${e.target.value}`)
	...
	useEffect(() => {
    	...
    	async function getLeadList() {
        	try {
				...
				// Assignee List
				try {
					const selAssApi = await axios.get(`https://task.mycrmdesk.com/backend/api/admin/get_sel_assignee_by_project/${allLeadsApi.data.project}`)
					setAssigneeList(selAssApi.data.data);
