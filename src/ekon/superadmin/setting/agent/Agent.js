...
const Agent = () => {
  ...
  async function onFormSubmit(e) {
		...
		if(agentName.name == ''){
			...
			try {
			axios.post(`https://task.mycrmdesk.com/backend/api/admin/add-agent`, agentName)
			.then((res) => {
				console.log(res);
	...
	useEffect(() => {
		...
		async function getAgentList(page) {
			page = page;
			try {
				const agentListApi = await axios.get(`https://task.mycrmdesk.com/backend/api/admin/agents?page=`+page)
				  		...
				  		<div className='col-md-12 col-xs-12'>
							<FormGroup
								...
								<select className="form-control" name="lead_by" id="lead_by" onChange={e => onTextFieldChange(e)} required>
									<option value={agentName.lead_by}>Select</option>
									{
