...
const EditCanva = (props) => {
	...
	async function getAgents() {
		try {
			const agentApi = await axios.get(`${BASE_URL}/admin/edit-agent/${userId}`);
			setAgents(agentApi.data);

			// Sel Subject list
			try {
				const selSubjectListApi = await axios.get(
					`${BASE_URL}/admin/get_sel_skills/${agentApi.data.skills}`,
				);
	...
	async function onFormSubmit(e) {
		e.preventDefault();
		try {
			axios.put(`${BASE_URL}/admin/update-agent/${userId}`, agents).then((res) => {
		...
		<>
			...
			<OffCanvas
				...
				<OffCanvasHeader setOpen={headerClose ? setOffcanvasStatus : undefined}>
					<OffCanvasTitle id='offcanvasExampleLabel'>Update Member</OffCanvasTitle>
				</OffCanvasHeader>
				<OffCanvasBody>
