...
const Dashboard = () => {
	...
	async function getUsers() {
		try {
			const userApi = await axios.get(`https://task.mycrmdesk.com/backend/api/admin/all-agent-list-data`);
				...
				<div id='bootstrap' className='row scroll-margin'>
					<div className='col-md-12'>
						<Card stretch>
							...
							<CardBody className=''>
								<iframe height={500} width='100%' onLoad={e => resizeIframe(e, this)} src="https://task.mycrmdesk.com/report"></iframe>
							</CardBody>
						</Card>
										...
										<div className='col-md-2 col-xs-12' key={index}>
											<Card stretch>
												...
												<CardFooter className=''>
													<Link to={'/superadmin/user-task/'+item.id}>Task List</Link>
												</CardFooter>
											</Card>
										</div>
									)) :
									...
									departmentCount.map((item,index)=>(
										<div className='col-md-2 col-xs-12' key={index}>
											<Card stretch>
												...
												<CardFooter className=''>
													<Link to={'/superadmin/project-task/'+item.project}>Task List</Link>
												</CardFooter>
											</Card>
										</div>
									)) :
									...
									statusCount.map((item,index)=>(
										<div className='col-md-2 col-xs-6' key={index}>
											<Card stretch>
												...
												<CardFooter className=''>
													<Link to={'/superadmin/status-task/'+item.status}>Task List</Link>
												</CardFooter>
											</Card>
										</div>
									)) :
									...
									categoryCount.map((item,index)=>(
										<div className='col-md-2 col-xs-6' key={index}>
											<Card stretch>
												...
												<CardFooter className=''>
													<Link to={'/superadmin/category-task/'+item.category}>Task List</Link>
												</CardFooter>
											</Card>
										</div>
									)) :
