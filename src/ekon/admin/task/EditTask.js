...
const EditTask = () => {
		...
		if (e.target.name == 'project') {
			try {
				const selAssApi = await axios.get(
					`${BASE_URL}/admin/get_sel_assignee_by_project_team_lead/${e.target.value}`,
				);
	...
	async function onFormSubmit(e) {
		...
		if (lead.name == '') {
			...
			try {
				axios.post(`${BASE_URL}/admin/update-lead/${id}`, lead).then((res) => {
	...
	useEffect(() => {
		...
		async function getAssigneeList() {
			try {
				const assigneeListApi = await axios.get(`${BASE_URL}/admin/all-agent-list_admin/${userIds}`)
		...
		*/
		...
		async function getDepartmentList() {
			try {
				const depApi = await axios.get(
					`${BASE_URL}/admin/all-project-list?lead_by=${userIds}`,
				);
	...
	useEffect(() => {
		async function getLeadList() {
			try {
				...
				try {
					const selAssApi = await axios.get(
						`${BASE_URL}/admin/get_sel_assignee_by_project_team_lead/${allLeadsApi.data.project}`,
					);
					setAssigneeList(selAssApi.data);
				} catch (error) {
					console.log('Something is Wrong -Sel Skills List');
