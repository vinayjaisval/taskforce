...
const Agent = () => {
	...
	async function onFormSubmit(e) {
		...
		if (agentName.name == '') {
			...
			try {
				axios.post(`${BASE_URL}/admin/add-agent_admin/${id}`, agentName).then((res) => {
					console.log(res);
	...
	useEffect(() => {
		async function getAgentList(page) {
			page = page;
			try {
				const agentListApi = await axios.get(
					`${BASE_URL}/admin/agents_admin/${id}?page=` + page,
				);
	...
	async function handleClick(e, delId) {
		axios.get(`${BASE_URL}/admin/agent_delete/${delId}`).then((res) => {
			getPaginatedData(1);
